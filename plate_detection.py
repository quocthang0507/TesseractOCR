...
class PlateDetector():
    def __init__(self, type_of_plate, minPlateArea, maxPlateArea):
        ...
        if (type_of_plate == 'SQUARE_PLATE'):
            self.element_structure = cv2.getStructuringElement(
                shape=cv2.MORPH_RECT, ksize=(26, 5))
    ...
    def find_characters_on_plate(self, plate):
        if (self.type_of_plate == 0):  # rectangle plate
            ...
            plate_lower = plate[plate.shape[0] /
                                2: plate.shape[0], 0:plate.shape[1]]

            # get the characters of the upper plate and the lower plate
            upper_charactersFound = segment_characters_from_plate(
                plate_upper, 200)
            lower_charactersFound = segment_characters_from_plate(
                plate_lower, 200)
    ...
    def extract_contours(self, after_preprocess):
        _, extracted_contours, _ = cv2.findContours(
