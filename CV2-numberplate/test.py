...
directory = r'D:\Github\quochuy\Doan\ALPR_System\photo_cut'
...
def concat_chars(list_char_on_plate, filename):
    """
    input is a list that contains a images of the same plate
    get the best images in the list
    calculates the average plate
    """
    temp = []
    list_char_on_plate = get_best_images(list_char_on_plate, num_img_return=1) # get the best images

    for segmented_characters in list_char_on_plate:
        temp.append( segmented_characters[1])
    line = cv2.hconcat([temp])
...
if __name__=="__main__":
    ...
    while(cap.isOpened()):
        ...
        cropped_frame = frame[int(frame_height*0.3*0.4):frame_height, 0:int(frame_width*0.9)] # crop the ROI
        cv2.rectangle(_frame, (200, int(_frame_height*0.3*0.4)), (int(_frame_width*0.8), _frame_height), (255, 0, 0), 2) # draw a
        ...
        if possible_plates is not None:
            ...
            if (distance < 100):
                if(countPlates < countPlates_threshold):
                    ...
                    i += 1
                    cv2.imwrite(os.path.join(directory,str(i) +".jpg"),possible_plates[0])
                    temp = []
                    ...
                    concat_chars(list_char_on_plate, os.path.join(directory,"chars_"+str(i) +".jpg"))
                    countPlates += 1
            else:
                countPlates = 0
                list_char_on_plate = []
